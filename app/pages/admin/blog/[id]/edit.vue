  ...
  <div class="container py-10">
     <admin-breadcrumbs
          ...
          :items="[
            ...
            {label: 'Edit Blog Post', to: `/admin/blog/${id}/edit`}
          ]"
        />
...
const { data, error } = await useFetch<NewBlogTypes>(
  `/api/admin/blog/${id}`
);
if (error.value)
  throw createError({ ...error.value, message: error.value.statusMessage });

const { form, reset } = useForm<NewBlogTypes>(data.value as NewBlogTypes);
